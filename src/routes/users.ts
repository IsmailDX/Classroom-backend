...
router.get('/', async (req, res) => {
  try {
    ...
    const usersList = await db.select().from(user).where(whereClause).orderBy(desc(user.createdAt)).limit(limitPerPage).offset(offset);
...
router.get('/:id/departments', async (req, res) => {
  try {
    ...
    const [userRecord] = await db.select({ id: user.id, role: user.role }).from(user).where(eq(user.id, userId));
